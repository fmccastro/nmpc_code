...
"""
    Script to evaluate baseline one-stage ocp with and without closest point optimization before simulation
"""
...
if __name__ == '__main__':
    ...
    model = Dynamics(x_ref, y_ref, yaw_ref)
    ...
    for initialPose in listPoses:
        ...
        if(saveData):
            if(closestPointComp):
                with open(common.results_folder + "One stage OCP/Closest point analysis/" + "initial_point_" + str(initialPose[0]) + " " + str(initialPose[1]) + "closest_point_comparison+" + "baseline_std.json", "w") as f:
                ...
                if(closestPoint):
                    with open(common.results_folder + "One stage OCP/Closest point analysis/" + "closest_point+" + "baseline_std.json", "w") as f:
                        data2save = { 'cost': cost, 'opt_time': opt_time, 'x': list( states[:, 0] ), 'y': list( states[:, 1] ), 'progress': list( states[:, 3] ), 'progress_0': progress_sol }
                        json.dump(data2save, f, indent=4)
                
                else:
                    with open(common.results_folder + "One stage OCP/Closest point analysis/" + "no_closest_point+" + "baseline_std.json", "w") as f:
