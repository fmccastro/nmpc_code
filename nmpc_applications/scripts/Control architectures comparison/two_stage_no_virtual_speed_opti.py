...
if __name__ == '__main__':
    ...
    kin_model = KinematicsRatesPath()
    dyn_model = DynamicsPath()

    #   (x, y, yaw, s, vx, wz)
    kin_next_state = [ float( x_ref(0) ) + 0.5, float( y_ref(0) ) - 1.0, float(yaw_ref(0)) + ca.pi/4, 0.0, 0.0]
    ...
    #   (x, y, yaw, vx, wz)
    dyn_next_state = [kin_next_state[0], kin_next_state[1], kin_next_state[2], kin_next_state[3], kin_next_state[4]]
    ...
    for i in np.arange(range_aux):
        ...
        if( i == 0 ):
            ...
            kin_controls = np.stack( [0.0, 0.0] )
            
            kin_res = kin_model._solve( kin_next_state, None, pitch, x_r, y_r )
            kin_prev_res = kin_res
        
        elif( i > 0 ):
            kin_states = np.vstack( ( kin_states, np.stack(kin_next_state) ) )
            kin_controls = np.vstack( ( kin_controls, np.stack( [ vx_rate_horizon[0], wz_rate_horizon[0] ] ) ) )

            kin_res = kin_model._solve( kin_next_state, kin_prev_res, pitch, x_r, y_r )
            kin_prev_res = kin_res
        ...
        vx_horizon = kin_res[3]
        wz_horizon = kin_res[4]
        vx_rate_horizon = kin_res[5]
        wz_rate_horizon = kin_res[6]
        ...
        kin_next_state = [ x_dyn_horizon[1], y_dyn_horizon[1], yaw_dyn_horizon[1], vx_dyn_horizon[1], wz_dyn_horizon[1] ]
    ...
    ax[1, 0].plot( kin_states[:, 4] )
    ax[1, 0].set_title('wz')

    ax[1, 1].plot( kin_controls[:, 0] )
    ax[1, 1].set_title('vx_rate')

    ax[1, 2].plot( kin_controls[:, 1] )
    ax[1, 2].set_title('wz_rate')

    ax[1, 3].plot( kin_states[:, 0], kin_states[:, 1] )
    ...
    data2save = { 'dyn_cost': dyn_cost, 'kin_cost': kin_cost, 'dyn_opt_time': dyn_opt_time, 'kin_opt_time': kin_opt_time,\
                  'x': list( kin_states[:, 0] ), 'y': list( kin_states[:, 1] ) }

    with open(common.results_folder + "Control_architectures/" + "two_stage_std_no_virtual_speed.json", "w") as f:
