...
if __name__ == '__main__':
    ...
    common = Common()
    obs = ObstacleAvoidance(x_len, y_len, 0.02, 0.02)
    ref = Reference()
    ...
    obs._check_sdf(sdf_sym)
    
    print("Symbolic SDF set.\n")
    ###

    #   Set up nmpc countoring model
    model = DynamicsMPCC(x_ref, y_ref, yaw_ref, sdf=sdf_sym)
    ...
    for initialPose in listPoses:
        ...
        ax_anim.contourf(X, Y, obs_map, cmap="binary_r")
        ax_anim.set( xlim=[-x_len, x_len], ylim=[-y_len, y_len], xlabel='X [m]', ylabel='Y [m]' )
