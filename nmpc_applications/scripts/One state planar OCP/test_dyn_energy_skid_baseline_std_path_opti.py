...
"""
    ...
    This script takes the solver that accounts for the control effort minimization.

    In this architecture, the virtual speed is controlled outside the OCP
"""
...
if __name__ == '__main__':
    ...
    model = DynamicsEnergyPath(x_ref, y_ref, yaw_ref)
    ...
    param = { 'pitch': pitch, 'roll': roll, 'friction': friction, 'fl_min': fl_min, 'fl_max': fl_max, 'fr_min': fr_min, 'fr_max': fr_max,\
              'fl_prev': 0.0, 'fr_prev': 0.0, 'friction': friction }
    ...
    for i in np.arange(range_aux):
        ...
        error = ca.sqrt( ca.power( next_state[0] - x_ref(progress), 2) + ca.power( next_state[1] - y_ref(progress), 2) )
    ...
    if(saveData):
        if(change_N):
            with open(common.results_folder + "One stage OCP/Solver specs comparison/horizon/Virtual speed controlled outside OCP/" + str(specs["N"]) + ".json", "w") as f:
                data2save = { 'cost': cost, 'opt_time': opt_time, 'x': list( states[:, 0] ), 'y': list( states[:, 1] ), 'progress': progress, 'progress_0': progress_sol,\
                               'yaw': list( states[:, 2] ), 'fl': list( controls[:, 0] ), 'fr': list( controls[:, 1] ),\
                               'x_ref': x_ref( progress_list ).elements(), 'y_ref': y_ref( progress_list ).elements(), 'yaw_ref': yaw_ref( progress_list ).elements() }
                json.dump(data2save, f, indent=4)
            
            with open(common.results_folder + "One stage OCP/Solver specs comparison/horizon/Virtual speed controlled outside OCP/" + str(specs["N"]) + "_specs.json", "w") as f:
                json.dump(solver_param, f, indent=4)
        
        elif(change_Ts):
            with open(common.results_folder + "One stage OCP/Solver specs comparison/sampling time/Virtual speed controlled outside OCP/" + str(specs["Ts"]) + ".json", "w") as f:
                data2save = { 'cost': cost, 'opt_time': opt_time, 'x': list( states[:, 0] ), 'y': list( states[:, 1] ), 'progress': progress, 'progress_0': progress_sol,\
                               'yaw': list( states[:, 2] ), 'fl': list( controls[:, 0] ), 'fr': list( controls[:, 1] ),\
                               'x_ref': x_ref( progress_list ).elements(), 'y_ref': y_ref( progress_list ).elements(), 'yaw_ref': yaw_ref( progress_list ).elements() }
                json.dump(data2save, f, indent=4)
            
            with open(common.results_folder + "One stage OCP/Solver specs comparison/sampling time/Virtual speed controlled outside OCP/" + str(specs["Ts"]) + "_specs.json", "w") as f:
                json.dump(solver_param, f, indent=4)

        elif(change_weights):
            with open(common.results_folder + "One stage OCP/Solver specs comparison/weights/Virtual speed controlled outside OCP/" + controllerType + ".json", "w") as f:
                data2save = { 'cost': cost, 'opt_time': opt_time, 'x': list( states[:, 0] ), 'y': list( states[:, 1] ), 'progress': progress, 'progress_0': progress_sol,\
                              'yaw': list( states[:, 2] ), 'vx': list( states[:, 3] ), 'wz': list( states[:, 4] ), 'fl': list( controls[:, 0] ), 'fr': list( controls[:, 1] ),\
                              'x_ref': x_ref( progress_list ).elements(), 'y_ref': y_ref( progress_list ).elements(), 'yaw_ref': yaw_ref( progress_list ).elements() }
                json.dump(data2save, f, indent=4)
            
            with open(common.results_folder + "One stage OCP/Solver specs comparison/weights/Virtual speed controlled outside OCP/" + controllerType + "_specs.json", "w") as f:
                json.dump(solver_param, f, indent=4)